...
int main(){
    ...
    fread(mv.TDD[0],2*sizeof(char),1,archBinario); //Leo el tama�o del codigo y asigno al DS
    ...
    mascarasAInstruccuion(mv.memoria[mv.registros[5]],&operando1,&operando2,&operancion);

    while(mv.TDD[1] != mv.registros[5]){
